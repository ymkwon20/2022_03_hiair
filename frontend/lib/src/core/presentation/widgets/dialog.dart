...
class CurrentWorkOrderFilterDialog extends ConsumerWidget {
  const CurrentWorkOrderFilterDialog({
    Key? key,
    required this.filterKey,
  }) : super(key: key);
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final filterList = ref.watch(currentWorkOrderColumnFilterProvider);
    final currentFilterMap = ref.watch(currentWorkOrderListNotifier).filterMap;
    final textFieldController = TextEditingController();
    ...
    void _onTap(int index) {
      if (filterList[index].isSelected) {
        ...
        ref
            .read(currentWorkOrderListNotifier.notifier)
            .setFilter(filterKey, newFilterList);
      } else {
        final newFilterList = currentFilterMap[filterKey]?.toList() ?? [];
        ...
        ref.read(currentWorkOrderListNotifier.notifier).setFilter(
              filterKey,
              newFilterList,
            );
      }
